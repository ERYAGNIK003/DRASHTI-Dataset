...
def get_frame_to_image_map(image_dir):
    ...
    print("Total images are ",len(images),images[0],images[140],images[141])
...
def read_cvat_video_tracks(xml_file,image_dir,framewiseData,vidname):
    ...
    for track in root.findall("track"):
        ...
        for box in track.findall("box"):
            ...
            xmax = pts[:, 0].max()
            ymax = pts[:, 1].max()
            xtl1 = max(0, min(3839, xmin))
            xbr1 = max(0, min(3839, xmax))
            ytl1 = max(0, min(2159, ymin))
            ybr1 = max(0, min(2159, ymax))
...
vids=["DJI_0150"]
xmlfiles=os.path.join(".","XMLs")
outDir=os.path.join(".","cvat_out")
...
for vidname in vids:
    ...
    vidDir=os.path.join(outDir,vidname)
    os.makedirs(vidDir, exist_ok=False)
    
    xmlPath=os.path.join(xmlfiles,vidname,"annotations.xml") 
    imgPath=os.path.join(vidname)

    framewiseData={}
    tracks = read_cvat_video_tracks(xmlPath,imgPath,framewiseData,vidname)

    annDir=os.path.join(vidDir,"annotated")
    dotaDir=os.path.join(vidDir,"dota_lbls")
    ...
    for frm, annotations in framewiseData.items():
        ...
        for ann in annotations:
            ...
            #create a classiwse dir
            clsDir=os.path.join(outDir,cls)
            ...
            cropped=img[crop_reg[1]:crop_reg[3],crop_reg[0]:crop_reg[2]]
            if cropped.shape[0] == 0 or cropped.shape[1] == 0:
                print(f"Problem with video : {vidname} track : {track_id} frame : {frm}")
                continue
            cv2.imwrite(os.path.join(clsDir,f"{vidname}_{track_id}_{frm}.jpg"),cropped)
            
            cv2.putText(img,f"{int(track_id)}",cen,cv2.FONT_HERSHEY_SIMPLEX,0.6,(0, 0, 255),2,cv2.LINE_AA)
        resized = cv2.resize(img, (1280, 720))
        cv2.imwrite(os.path.join(annDir,frm+".jpg"),resized)
